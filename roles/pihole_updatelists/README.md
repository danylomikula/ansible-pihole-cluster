...
| `pihole_updatelists_blacklist_url` | `""` | URL for handcrafted exact-domain blacklist |
...
- `pihole_updatelists_blacklist_url` is only for handcrafted exact-domain lists. Do not point it at regular `hosts` or adblock lists such as `StevenBlack/hosts`.
- Use `pihole_updatelists_blocklists_url` for remote adlist sources.
